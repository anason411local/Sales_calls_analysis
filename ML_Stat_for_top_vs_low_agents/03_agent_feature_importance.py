...
df_target_corr = pd.read_csv(f'analysis_outputs/{AGENT_TYPE}/02_correlation_with_target.csv')
...
rf_importance.to_csv(f'analysis_outputs/{AGENT_TYPE}/03_importance_random_forest.csv', index=False)
xgb_importance.to_csv(f'analysis_outputs/{AGENT_TYPE}/03_importance_xgboost.csv', index=False)
df_combined.to_csv(f'analysis_outputs/{AGENT_TYPE}/03_importance_combined.csv', index=False)
...
# Save models
import joblib
joblib.dump(rf_model, f'analysis_outputs/{AGENT_TYPE}/03_model_random_forest.pkl')
joblib.dump(xgb_model, f'analysis_outputs/{AGENT_TYPE}/03_model_xgboost.pkl')
...
import json
with open(f'analysis_outputs/{AGENT_TYPE}/03_model_metrics.json', 'w') as f:
...
plt.tight_layout()
plt.savefig(f'analysis_outputs/{AGENT_TYPE}/03_eval_confusion_matrices.png',
...
plt.tight_layout()
plt.savefig(f'analysis_outputs/{AGENT_TYPE}/03_eval_roc_curves.png',
            dpi=300, bbox_inches='tight')
...
plt.suptitle('LEVEL 1: LEARNING CURVES\nGap between training and validation = Overfitting level',
             fontsize=14, fontweight='bold', y=1.02)
plt.tight_layout()
plt.savefig(f'analysis_outputs/{AGENT_TYPE}/03_eval_learning_curves.png',
            dpi=300, bbox_inches='tight')
...
plt.tight_layout()
plt.savefig(f'analysis_outputs/{AGENT_TYPE}/03_eval_metrics_comparison.png',
            dpi=300, bbox_inches='tight')
